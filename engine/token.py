...
class Token:
    ...
    # --- POMOCNICZE: diagnostyka możliwości ruchu ---
    def get_movement_points(self) -> int:
        """Zwraca aktualne punkty ruchu (bieżące)."""
        return int(getattr(self, 'currentMovePoints', self.stats.get('move', 0)) or 0)

    def get_fuel(self):
        """Zwraca aktualne paliwo, jeśli jednostka korzysta z paliwa; w przeciwnym razie None."""
        if hasattr(self, 'currentFuel'):
            return getattr(self, 'currentFuel', None)
        # Jeśli brak pola maintenance w stats – traktuj jako brak systemu paliwa
        if self.stats.get('maintenance', None) in (None, 0):
            return None
        return getattr(self, 'currentFuel', None)

    def can_move_reason(self):
        """Zwraca (can_move: bool, reason: str). reason ∈ {'ok','no_mp','no_fuel'}"""
        mp = self.get_movement_points()
        if mp <= 0:
            return False, 'no_mp'
        fuel = self.get_fuel()
