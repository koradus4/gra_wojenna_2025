...
class PanelDowodcy:
    ...
    def uzupelnij_zeton(self, token, max_fuel_do_uzupelnienia, max_combat_do_uzupelnienia, max_punkty, callback=None):
        ...
        def zatwierdz():
            ...
            # Odśwież UI po uzupełnieniu: panel mapy (markery paliwa/MP) i panel informacji o żetonie
            try:
                if hasattr(self, 'panel_mapa') and self.panel_mapa is not None:
                    # Aktualizuj widok mapy i markery statusu ruchu
            ...
            try:
                ...
                details = {
                    'token_id': token.id,
                    'to_q': getattr(token, 'q', None), 'to_r': getattr(token, 'r', None),
                }
                log_action(
                    self.game_engine,
                    self.gracz,
                    ...
                    'resupply',
                    details=details,
                    result_msg=f"fuel+{ile_fuel}, combat+{ile_combat}"
                )
            except Exception:
                pass
            win.destroy()
